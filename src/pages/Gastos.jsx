...
import { expenseCategories, paymentMethods } from '../app/constants.js'
...
export default function Gastos() {
  ...
  const filtered = useMemo(() => {
    return rows.filter((row) => {
      const matchesQuery = String(row.descripcion || row.description || '')
        .toLowerCase()
        .includes(query.toLowerCase())
      const matchesCategory = category ? row.categoria === category : true
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 rounded-xl border border-slate-800/50 bg-slate-950/60 p-5 shadow-soft lg:grid-cols-2">
